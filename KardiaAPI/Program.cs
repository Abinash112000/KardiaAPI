...
string EncryptionKey = builder.Configuration.GetSection("Kardia")["EncryptionKey"].ToString();
...
app.MapPost("/KardiaApi/Login" , [AllowAnonymous] async(string username, string password) =>
{
    ...
    string pass = BllSecurity.DecryptPassword(password, EncryptionKey);
...
async ([FromBody] CallBackModel value) =>
{
    ...
    try
    {
        ...
        string passwrd = Password.Replace("+", "|PLUS|");
        ...
        string urlforSaveData = servicesURL + $"/Services/IntegrationService/Integration.ProtocolServices/Webhooks/SaveKardiaData";
